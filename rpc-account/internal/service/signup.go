...
func (s *AccountService) SignUp(ctx context.Context, req *account.SignUpReq) (*empty.Empty, error) {
	...
	if err == redis.Nil {
		return nil, grpcex.NewErrorf(codes.InvalidArgument, requestID, "InvalidArgument", "captcha is not exists")
	}
	if err != nil {
		return nil, errors.Wrapf(err, "redis get key [%v] failed", key)
	}
	if req.Captcha != val {
		return nil, grpcex.NewErrorf(codes.InvalidArgument, requestID, "InvalidArgument", "captcha is not match")
	}

	birthday, err := cast.ToTimeE(req.Birthday)
	if err != nil {
		return nil, grpcex.NewErrorf(codes.InvalidArgument, requestID, "InvalidArgument", "invalid birthday format")
	}
		...
		case *mysql.MySQLError:
			...
			if e.Number == 1062 {
				if strings.Contains(e.Message, "accounts.email_idx") {
					return nil, grpcex.NewErrorf(codes.InvalidArgument, requestID, "InvalidArgument", "account [%v] exists", req.Email)
				}
				if strings.Contains(e.Message, "accounts.phone_idx") {
					return nil, grpcex.NewErrorf(codes.InvalidArgument, requestID, "InvalidArgument", "account [%v] exists", req.Phone)
				}
				if strings.Contains(e.Message, "accounts.name_idx") {
					return nil, grpcex.NewErrorf(codes.InvalidArgument, requestID, "InvalidArgument", "account [%v] exists", req.Name)
				}
			}
		}
